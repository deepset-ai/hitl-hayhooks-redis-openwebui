...
| `DEMO_MODE` | `true` | When `true`, `submit_feedback_to_deepset` simulates posting instead of hitting the real Slack webhook. Set to `false` to actually post |
| `SLACK_WEBHOOK_URL` | - | Slack Incoming Webhook URL to post feedback to (required if `DEMO_MODE=false`) |
